...
class VAE(nn.Module):
    def __init__(self, in_channels=1, bottleneck=64, img_height=28, img_width=28):
        ...
        self.mean = nn.Linear(self.conv_out_shape, bottleneck)
        self.logvar = nn.Linear(self.conv_out_shape, bottleneck)
    ...
    @torch.no_grad()
    def _out_size(self):
        x = torch.zeros(1, self.in_channels, self.h, self.w).to(device)
        x = self.encoder(x)
