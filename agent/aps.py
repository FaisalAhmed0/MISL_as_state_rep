...
class APSAgent(DDPGAgent):
    def __init__(self, update_task_every_step, sf_dim, knn_rms, knn_k, knn_avg,
                 knn_clip, num_init_steps, lstsq_batch_size, update_encoder=True,
                 **kwargs):
        self.sf_dim = sf_dim
        ...
        kwargs["update_encoder"] = update_encoder
    ...
    def compute_intr_reward(self, task, next_obs, step):
        # maxent reward
        with torch.no_grad():
            rep = self.aps(next_obs, norm=False) # the state features
        reward = self.pbe(rep)
